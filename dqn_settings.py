...
from config_settings import DEFAULT_LOG_DIR, GLOBAL_STATE_UPDATE_CYCLE
import os
...
flags.DEFINE_string('sumo_connect_lib', 'traci', 'choose traci or libsumo')
flags.DEFINE_boolean('sumo_gui', False, 'Enable sumo-gui')
flags.DEFINE_integer('num_workers', 0, 'number of worker')
...
flags.DEFINE_integer('vehicles', 500, "number of vehicles")
...
NUM_FEATURES = 7 + NUM_SUPPLY_DEMAND_MAPS * (1 + (FLAGS.n_diffusions + 1) * 2) + FLAGS.n_diffusions * 2 + FLAGS.trip_diffusion * 4

# training hyper parameters
WORKING_COST = 0.2
DRIVING_COST = 0.2
STATE_REWARD_TABLE = {
    status_codes.IDLE : -(WORKING_COST + DRIVING_COST),
    status_codes.ASSIGNED : -(WORKING_COST + DRIVING_COST),
}
